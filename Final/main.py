...
#             for m in self.models:
#                 if not m.enabled or self.cached_predictions[m.name] is None: continue
#                 try:
#                     frame = m.draw(frame, self.cached_predictions[m.name])
#                 except Exception as e:
#                     print(f"⚠️ Draw error in {m.name}: {e}")
#             cv2.imshow("Multi-Model Vision", frame)
...
#     print("\n Multi-Model Vision Pipeline: Face + Activity + Background\n")
...
#         self.snapshot_interval = 3.0  # seconds
